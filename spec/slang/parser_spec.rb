...
describe Parser do
  ...
  it_parse [:fun, :foo, [], [:ret, 1]], Function.new(:foo, [], [Return.new([1.int])])
	it_parse [:external, :puts, [:String], :Integer], External.new(:puts, [Paramter.new(nil, :String)], :Integer)
	it_parse [:class, :Integer], ClassDef.new(:Integer)
	it_parse [:operator, :+, [:Integer, :Integer], :Integer], Operator.new(:+, [Paramter.new(nil, :Integer), Paramter.new(nil, :Integer)], :Integer)
