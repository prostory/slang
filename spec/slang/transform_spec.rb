...
describe SLang::Transform do
  ...
  context "statement" do
    ...
    it "transforms a variable" do
      expect(xform.apply(:ident => 'hello')).to eq('hello'.var)
    end

    it "transforms a negative expression" do
      input = parser.parse('-i')
      expect(xform.apply(input)).to eq('-'.call(['i'.var], 0.int))
    end

    it "transforms a unary operation" do
      input = parser.parse('!i')
      expect(xform.apply(input)).to eq('!'.call([], 'i'.var))
    end

    it "transforms a binary operation" do
      input = parser.parse('1 +i')
      expect(xform.apply(input)).to eq('+'.call(['i'.var], 1.int))
    end

    it "transforms a if statement" do
      input = parser.parse('if i < 0 then i else -i end')
      expect(xform.apply(input)).to eq(If.new('<'.call([0.int], 'i'.var), 'i'.var, '-'.call(['i'.var], 0.int)))
    end
    ...
    it "transforms a if statement witch has else if subtree" do
      ...
      expect(xform.apply(input)).to eq(If.new('<'.call([0.int], 'i'.var), 'i'.var,
    ...
    it "transforms a unless statement" do
      ...
      expect(xform.apply(input)).to eq(If.new('!'.call([], '<'.call([0.int], 'i'.var)), 'i'.var))
    end

    it "transforms a case of statement" do
      input = parser.parse('case a of 1, 2; 3 of 3; 4 else 5 end')
      expect(xform.apply(input)).to eq(If.new('=='.call([1.int], 'a'.var), 3.int,
    ...
    it "transforms a module declaration include function" do
      ...
      expect(xform.apply(input)).to eq(mod)
    end

    it "transforms a class declaration" do
      input = parser.parse('class A < B a = a + 2; 3 end')
      expect(xform.apply(input)).to eq('A'.class_def([Assign.new('a'.var, '+'.call([2.int], 'a'.var)), 3.int], 'B'))
    end
    ...
    it "transforms a class declaration include function" do
      ...
      expect(xform.apply(input)).to eq(clazz)
    end

    it "transforms a function declaration" do
      input = parser.parse('def foo(a, b); a + b end')
      expect(xform.apply(input)).to eq('foo'.function(['a'.param, 'b'.param], '+'.call(['b'.var], 'a'.var)))
    end

    it "transforms a external declaration" do
      input = parser.parse('external foo(a, b);')
      expect(xform.apply(input)).to eq('foo'.external(['a'.param, 'b'.param]))
    end

    it "transforms a operator declaration" do
      input = parser.parse('operator +(a, b);')
      expect(xform.apply(input)).to eq('+'.operator(['a'.param, 'b'.param]))
    end
  end
end